...
class SorcererFactoryTest {
    private Field field;
    @Test
    public void TestSorcererFactory() {
        field = new Field();
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
        ...
        tactician1.changeFactory(SorcererFactory);
        tactician1.addUnit(100, 1, 0, 0);
        assertTrue(tactician1.getSelectedUnit() instanceof Sorcerer);
        tactician2.changeFactory(SorcererFactory);
